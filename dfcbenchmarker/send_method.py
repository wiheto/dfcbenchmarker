...
def send_method(method_code,param_code,author_name=None,method_name=None,email_address=None,publications=None,agree_dfcbenchmarker=None,agree_teneto=None,email_list=None):
    ...
    privacy = input('All data provided here is saved by DFC Benchmarker. It is sent via Google Forms and the data provided will be stored on a (private) Google Spreadsheet. By continuing you agree to this. Press "y" and enter to continue: ')
    ...
    if not agree_dfcbenchmarker:
        agree_dfcbenchmarker = input('Agree for DFC Benchmarker to use code in future releases (with credit to published reference) [Yes or No]: ')

    if agree_dfcbenchmarker.capitalize() != 'Yes' and agree_dfcbenchmarker.capitalize() != 'Y':
        raise ValueError('Must agree that DFC Benchmarker can use the code if sending to DFC Benchmarker')

    if not email_list:
        email_list = input('Do you want to receive updates from DFC Benchmarker (e.g. when we release new reports)? [OPTIONAL] [Yes or No]: ')
