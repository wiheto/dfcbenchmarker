...
def run_simulations(routine_version=1.0,usesaved='yes',new_method=None,params_new_method=None,output_dir=None):
    ...
    if params_new_method:
        ...
        params_new_method = dfcbenchmarker.check_params(params_new_method,'dfc')
    ...
    if output_dir == None:
        output_dir = './dfcbenchmarker/simulation/' + str(routine_version) + '/'
    ...
    for sim_i in range(0,len(params['simulation'])):
        ...
        if usesaved == 'yes':
            data = dfcbenchmarker.load_data(sim['name'],len(sim['multi_index'])+1)
            dfc = dfcbenchmarker.dfc_calc(sim['name'],col_ind=len(sim['multi_index'])+1)
        else:
            ...
            data = dfcbenchmarker.gen_data(sim)
            dfc = pd.DataFrame(index=data.index)
            for dfc_i in range(0,len(params['dfc'])):
                calcdfc = dfcbenchmarker.dfc_calc(data,methods=params['dfc'][dfc_i]['method'],**params['dfc'][dfc_i]['params'],mi=multi_index_labels)
        ...
        if new_method:
            for i,nm in enumerate(new_method):
                dfc_new = dfcbenchmarker.calc_new_method(data,nm,params_new_method['dfc'][i])
        ...
        if sim['name'] == 'sim-1':
            ...
            dfcbenchmarker.plot_timeseries(data,plot_autocorr='yes',mi=[],fig_dir=fig_dir,fig_prefix=sim['name'])
            dfcbenchmarker.plot_dfc_timeseries(dfc,mi=[],fig_dir=fig_dir,fig_prefix=sim['name'])
            dfcbenchmarker.plot_method_correlation(dfc,mi=[],fig_dir=fig_dir,fig_prefix=sim['name'])

        elif sim['name'] == 'sim-2' or sim['name'] == 'sim-3' or sim['name'] == 'sim-4':


            dfcbenchmarker.plot_timeseries(data,plot_autocorr='no',fig_dir=fig_dir,fig_prefix=sim['name'],mi=sim['multi_index'])
            dfcbenchmarker.plot_fluctuating_covariance(data,fig_dir=fig_dir,fig_prefix=sim['name'],mi=sim['multi_index'])
            dfc=dfc.dropna()
            dfcbenchmarker.model_dfc(data,dfc,stat_dir,sim['name'],mi=sim['multi_index'],model_params=params['stats']['trace'])
            dfcbenchmarker.calc_waic(dfc,model_dir=stat_dir,save_dir=table_dir,file_prefix=sim['name'],burn=params['stats']['burn'],mi=sim['multi_index'])

            dfcbenchmarker.plot_betadfc_distribution(dfc,dat_dir=stat_dir,fig_dir=fig_dir,model_prefix=sim['name'],burn=params['stats']['burn'],mi=sim['multi_index'])
